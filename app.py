...
def read_all_sheets():
    """Read all app sheets with one batch request and retry 429 safely."""
    ss = get_spreadsheet()
    spreadsheet_id = st.secrets["spreadsheet_id"]
    ...
    for attempt in range(4):
        try:
            # One HTTP read request for all worksheets.
            ...
            if status != 429 and "429" not in str(exc):
                raise
            # Exponential backoff. Do not hammer the API while quota is full.
            if attempt == 3:
                raise
            delay = (2 ** attempt) * 3 + random.uniform(0, 1.5)
    ...
    # The API normally returns one valueRange per requested range.
    # Keep the mapping positional and fall back to empty data if a range is empty.
    ...
    for attempt in range(4):
        try:
            ...
            if "429" not in str(exc) or attempt == 3:
                raise
            time_module.sleep((2 ** attempt) * 3 + random.uniform(0, 1.5))
...
try:
    # One cached batch read for all seven application sheets.
    all_data = read_all_sheets()
    schools = all_data["schools"].copy()
    teachers = all_data["teachers"].copy()
    classes = all_data["classes"].copy()
    schedules = all_data["schedules"].copy()
    attendance = all_data["attendance"].copy()
    kpi = all_data["kpi"].copy()
    incidents = all_data["incidents"].copy()
except Exception as e:
    if "429" in str(e) or "Quota exceeded" in str(e):
        st.warning(
            "Google Sheets đang tạm giới hạn số lần đọc (429). "
            "App đã tự giảm request và sẽ tự thử lại. Nếu vừa deploy/reload nhiều lần, "
            "hãy chờ khoảng 1–2 phút rồi tải lại trang."
        )
        st.stop()
