...
def read_sheets(sheet_names_tuple):
...
def read_sheet(sheet_name):
    return read_sheets((sheet_name,))[sheet_name].copy()
...
def refresh_now():
    """Clear cached Google Sheets data and force a fresh read on rerun."""
    clear_cache()
...
try:
    loaded = read_sheets(tuple(PAGE_SHEETS[page]))
    ...
    if not schools.empty:
        ...
        if st.button("🗑️ Xóa trường đã chọn", type="secondary", width='stretch'):
            ...
            if has_classes:
                ...
                dep = read_sheets(("classes", "schedules"))
...
# =========================================================
    ...
    if not teachers.empty:
        ...
        if st.button("🗑️ Xóa giáo viên đã chọn", type="secondary", width='stretch'):
            ...
            dep = read_sheets(("schedules", "attendance", "kpi"))
